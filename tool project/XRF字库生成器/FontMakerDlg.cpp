...
void CFontMakerDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LIST_CHARSETS, m_listCharsets);
	DDX_Control(pDX, IDC_BTN_EDIT, m_btnEdit);
	DDX_Control(pDX, IDC_SPIN1, m_spFontWidth);
	DDX_Control(pDX, IDC_SPIN7, m_spFontWidth2);
...
BEGIN_MESSAGE_MAP(CFontMakerDlg, CDialog)
	...
	ON_CBN_SELCHANGE(IDC_LIST_CHARSETS, &CFontMakerDlg::OnCbnSelchangeListCharsets)
	...
	ON_EN_CHANGE(IDC_EDIT_WIDTH2, &CFontMakerDlg::OnEnChangeEditWidth2)
...
// 叶飞 : 加载内置码表
void CFontMakerDlg::OnCharsetChange_xLeaves()
{
	UINT count;
	CString name;
	m_listCharsets.GetWindowText(name);
	name.Append(L".cst");
	name = m_szCharsetPath + name;
	charsetIndex = m_listCharsets.GetCurSel();
	m_charset.Delete();
	if (!m_charset.LoadFromFile(name))
	{
		m_nCharIndex = 0;
		MessageBox(L"加载字符集文件失败!", name, MB_OK | MB_ICONWARNING);
		return;
	}
	count = m_charset.GetCharCount();
	m_nCharIndex = 0;
...
BOOL CFontMakerDlg::OnInitDialog()
{
	...
	m_listCharsets.AddString(L"ANSI");
	m_listCharsets.AddString(L"UCS2");
	m_listCharsets.AddString(L"GB2312");
	m_listCharsets.SetCurSel(1);
	OnCharsetChange_xLeaves();
	...
	*/
	...
	m_spFontSize.SetRange(1, 64);
	m_spFontWidth.SetRange(1, 32);
	m_spFontWidth2.SetRange(1, 32);
	m_spFontHeight.SetRange(1, 32);
	m_spOffsetX.SetRange(-999,+999);
	m_spOffsetY.SetRange(-999,+999);

	SetDlgItemInt(IDC_EDIT_WIDTH,16);
	SetDlgItemInt(IDC_EDIT_WIDTH2, 8);
...
void CFontMakerDlg::OnCbnSelchangeListCharsets()
{
	OnCharsetChange_xLeaves();
}
...
void CFontMakerDlg::OnEnChangeEditWidth2()
{
	if (m_bInitOK)
	{
		m_nFontWidth2 = GetDlgItemInt(IDC_EDIT_WIDTH2);
		if (m_nFontWidth2 > 32)
		{
			m_nFontWidth2 = 32;
			SetDlgItemInt(IDC_EDIT_WIDTH2, 32);
		}
	}
}
...
void CFontMakerDlg::OnBnClickedBtnSave()
{
    ...
    CString name;
    CFileMaker maker;
	CFileDialog fbox(FALSE, L"xrf");
	scan = BST_UNCHECKED;	// 默认水平扫描 [改为 BST_CHECKED 则为垂直扫描]
	msb  = BST_UNCHECKED;		// 默认高位在前 [改为 BST_UNCHECKED 则为低位在前]
    var  = BST_UNCHECKED;	// 默认固定字宽 [改为 BST_CHECKED 则为可变字宽]
	...
	if(fbox.DoModal()==IDOK)
	{
		...
		*/
		maker.MakeXrfFile(&m_bitfont, &m_charset, &file, scan, msb, var, charsetIndex, m_nFontWidth, m_nFontWidth2, m_nFontHeight);
		file.Close();
