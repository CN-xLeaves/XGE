...
Namespace xge
	...
	Declare Function LoadGDI_Memory(DPtr As Any Ptr,DSize As Integer) As Surface
	Declare Function LoadGDI(FileName As ZString Ptr) As Surface
	Declare Function LoadXPK(XpkObj As xywhPack Ptr,SID As Integer,PassWord As ZString Ptr) As Surface
...
Namespace xge
	...
	#Ifdef XGE_USE_GDI
		Function LoadGDI_Memory(DPtr As Any Ptr,DSize As Integer) As Surface
	...
	#Ifdef XGE_USE_XPK
		Function LoadXPK(XpkObj As xywhPack Ptr,SID As Integer,PassWord As ZString Ptr) As Surface
			If XpkObj->CheckIndex(SID) Then
				Dim As Integer bpp,w,h,TempInt,tpe,MemSize		' 获取属性
				Dim BitMap As Any Ptr
				Dim TempSurface As Surface
				TempInt = XpkObj->File_GetExtByte(SID)
				bpp = IIf(TempInt And 16,32,16)											' 获取色深
				tpe = IIf(TempInt And 32,-1,0)											' 获取类型 [0=bitmap,1=GDI]
				If tpe Then				' GDI
					MemSize = XpkObj->File_Size(SID)
					BitMap = Allocate(MemSize)
					XpkObj->GetFileData(SID,0,MemSize,BitMap,PassWord,NULL)
					Return LoadGDI_Memory(BitMap,MemSize)
				Else							' bitmap
					TempInt = XpkObj->File_GetExtInt(SID)
					w = HiWord(TempInt)
					h = LoWord(TempInt)
					TempSurface = ImageCreate(w,h,,bpp)
					If TempSurface Then
						ImageInfo(TempSurface,,,,,BitMap,MemSize)
						XpkObj->GetFileData(SID,0,MemSize,BitMap,PassWord,NULL)
						Return TempSurface
