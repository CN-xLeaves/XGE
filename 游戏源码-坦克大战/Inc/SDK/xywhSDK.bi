...
Namespace rtl
	...
	Function Replace(ByVal Text as String,ByVal FStr as String,ByVal RStr as String) As String
		Dim Replace_Output As String
		Dim position As Long
		Dim Old_Position As Long
		Do
			Old_Position = position
			position = InStr(position + 1, lcase(Text), lcase(FStr))
			If 0 <> position Then
				Replace_Output = Replace_Output + Mid(Text, Old_Position + 1, (position - 1) - Old_Position)
				Replace_Output = Replace_Output + RStr
				position = position + Len(FStr) - 1
			Else
				Replace_Output = Replace_Output + Mid(Text, Old_Position + 1)
			End If
