	...
	Extern XGE_EXTERNMODULE
		Namespace xFile
			...
			Function Open(FilePath As ZString Ptr, OnlyRead As Integer = 0) As HANDLE XGE_EXPORT_ALL
				Dim FileHdr As HANDLE =  CreateFile(FilePath, IIf(OnlyRead, GENERIC_READ, GENERIC_READ Or GENERIC_WRITE), FILE_SHARE_READ, NULL, IIf(OnlyRead, OPEN_EXISTING, OPEN_ALWAYS), FILE_ATTRIBUTE_NORMAL, NULL)
				If FileHdr <> INVALID_HANDLE_VALUE Then
					Return FileHdr
				EndIf
			End Function
