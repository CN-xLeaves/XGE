...
xge.InitW(640, 480, XGE_INIT_WINDOW, "XGE - Font Driver")
...
Sub DrawWord_ANSI(fd As xge.FontDriver Ptr, sf As xge.Surface Ptr, px As Integer, py As Integer, w As Integer, h As Integer, iCode As UInteger, iColor As UInteger, Style As Integer)
...
Sub InfoWord_ANSI(fd As xge.FontDriver Ptr, iCode As Integer, Style As Integer, w As Integer Ptr, h As Integer Ptr)
...
Function MyFontLoad(fd As xge.FontDriver Ptr, Addr As ZString Ptr, iSize As UInteger, param As Any Ptr) As Integer
	...
	fd->DrawWordW = Cast(Any Ptr, @DrawWord_ANSI)
	fd->WordInfoW = Cast(Any Ptr, @DrawWord_ANSI)
	
	' these are unnecessary implementations and can be left blank
	fd->FreeFont = NULL
	fd->DrawLineW_Fast = NULL
	fd->DrawLineA_Fast = NULL
	fd->DrawRectW_Fast = NULL
...
xge.Text.LoadFontA("it's just something that doesn't exist anyway", 0)
