...
Function MapEditScene(msg As Integer, param As Integer, eve As XGE_EVENT Ptr) As Integer
	...
	Select Case msg
		Case XGE_MSG_DRAW					' 绘图
			...
			' 绘制说明菜单
			xge.Text.DrawA(NULL, 516,  20, "按 0-9 选取地形", 0, &HFFFFFFFF)
			img_MapTile->DrawEx_Trans(NULL, 500, 60, 0, 0, 15, 15)
			xge.Text.DrawA(NULL, 520,  62, "0.空白地形", 0, &HFFFFFFFF)
			img_MapTile->DrawEx_Trans(NULL, 500, 80, 16, 0, 15, 15)
			xge.Text.DrawA(NULL, 520,  82, "1.砖墙 [可以击碎]", 0, &HFFFFFFFF)
			img_MapTile->DrawEx_Trans(NULL, 500, 100, 32, 0, 15, 15)
			xge.Text.DrawA(NULL, 520,  102, "2.铁墙 [难以击碎]", 0, &HFFFFFFFF)
			img_MapTile->DrawEx_Trans(NULL, 500, 120, 48, 0, 15, 15)
			xge.Text.DrawA(NULL, 520,  122, "3.草地 [可以隐身]", 0, &HFFFFFFFF)
			img_MapTile->DrawEx_Trans(NULL, 500, 140, 64, 0, 15, 15)
			xge.Text.DrawA(NULL, 520,  142, "4.河流 [坐船通行]", 0, &HFFFFFFFF)
			img_MapTile->DrawEx_Trans(NULL, 500, 160, 80, 0, 15, 15)
			xge.Text.DrawA(NULL, 520,  162, "5.基地 [击中失败]", 0, &HFFFFFFFF)
			img_MapTile->DrawEx_Trans(NULL, 500, 180, 96, 0, 15, 15)
			xge.Text.DrawA(NULL, 520,  182, "6.基地 [击中失败]", 0, &HFFFFFFFF)
			img_MapTile->DrawEx_Trans(NULL, 500, 200, 112, 0, 15, 15)
			xge.Text.DrawA(NULL, 520,  202, "7.基地 [击中失败]", 0, &HFFFFFFFF)
			img_MapTile->DrawEx_Trans(NULL, 500, 220, 128, 0, 15, 15)
			xge.Text.DrawA(NULL, 520,  222, "8.基地 [击中失败]", 0, &HFFFFFFFF)
			img_MapTile->DrawEx_Trans(NULL, 500, 240, 144, 0, 15, 15)
			xge.Text.DrawA(NULL, 520,  242, "9.熔岩 [不可通行]", 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 505,  282, "当前选择地形 :", 0, &HFFFFFFFF)
			If SelTK < 10 Then
				img_MapTile->DrawEx_Trans(NULL, 596, 280, SelTK*16, 0, 15, 15)
			EndIf
			xge.Text.DrawA(NULL, 518,  320, "按 F1 玩家位置", 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 518,  340, "按 F3 选刷新点", 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 518,  360, "按 F4 设刷新点", 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 518,  380, "按 F8 填充地图", 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 518,  400, "按 F9 保存地图", 0, &HFFFFFFFF)
			' 绘制地图
			Map.Draw(0,0)
			' 绘制出生点
