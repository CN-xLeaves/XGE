...
Function GameScene(msg As Integer, param As Integer, eve As XGE_EVENT Ptr) As Integer
	Select Case msg
		...
		Case XGE_MSG_DRAW				' draw
			...
			' 绘制信息文字
			xge.Text.DrawRectA(NULL, 480, 452, 160, 28, "fps : " & xge.Scene.FPS, 0, &HFFFFFFFF)
			' 绘制坦克、地图、子弹等
			GameObject.DrawAll()
			' 绘制调试信息
			xge.Text.DrawA(NULL, 500,  20, "player1 :", 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 524,  40, "等级 : " & GameObject.Tank.Player.lv, 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 524,  60, "装甲 : " & GameObject.Tank.Player.hp & "/" & GameObject.Tank.Player.hpmax, 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 524,  80, "经验 : " & GameObject.Tank.Player.Exp & "/" & GameObject.Tank.Player.ExpMax, 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 524, 100, "点数 : " & GameObject.Tank.Player.pt, 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 524, 120, "移动 : " & GameObject.Tank.Player.slv, 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 524, 140, "装填 : " & GameObject.Tank.Player.llv, 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 524, 160, "攻击 : " & GameObject.Tank.Player.alv, 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 524, 180, "防御 : " & GameObject.Tank.Player.dlv, 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 500, 380, "AI : ", 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 524, 400, "活跃 : " & GameObject.Tank.GetCountAI, 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 524, 420, "剩余 : " & GameObject.Map.HeadInfo.NumAI, 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 500, 220, "操作 :", 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 524, 240, "F2 显示血条", 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 524, 260, "F3 显示等级", 0, &HFFFFFFFF)
			xge.Text.DrawA(NULL, 524, 280, "F4 显示血量", 0, &HFFFFFFFF)
			If CheatMode Then
				xge.Text.DrawA(NULL, 524, 300, "F5 玩家升级", 0, &HFFFFFFFF)
				xge.Text.DrawA(NULL, 524, 320, "F6 添加敌人", 0, &HFFFFFFFF)
				xge.Text.DrawA(NULL, 524, 340, "F7 恢复血量", 0, &HFFFFFFFF)
			EndIf
